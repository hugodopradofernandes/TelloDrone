...
# Scripts uses djitellopy wrapper (https://github.com/damiafuentes/DJITelloPy)
#   *Install from github, pip version has an error on "temp?" command
# Based on djitellopy example script (video + keyboard).
#
###--------------------------------------------------------------
###IMPORTS
...
#RUN FUNCTION
    def run(self):
        ...
        while not should_stop:
            ...
            #-------------------
            #FRAME/SCREEN UPDATE AND TASKS RELATED TO SCREEN IMAGE
            #CV2 draw frame
...
#-------------------
#QRCode
    def qrscancode(self, parameter):
                qrcode=decode(frame)
                for decodedObject in qrcode: 
                    ...
                    print(x, y, ' - Type: ', decodedObject.type, ' Data: ', decodedObject.data.decode("utf-8"))
                    cv2.putText(frame, barCode, (x, y), font, 0.7, (0,255,255), 1, cv2.LINE_AA)
#-------------------
#History
